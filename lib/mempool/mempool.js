...
class Mempool extends EventEmitter {
  ...
  getSlp(hash) {
    ...
    const key = Buffer.concat([prefix, hash]);
    const slpRecords = this.map.get(key);
  ...
  /**
   * Find all coins in mempool for a certain address (unspents only).
   * @param {Address} addr
   * @param {Object} options
  ...
  getCoinsByAddress(addr, options) {

    if (typeof addr === "string")
      addr = new Address(addr, this.network);
    
    const coins = [];

    const metas = this.addrindex.getMeta(addr, options);
    for (const meta of metas) {
      const outputs = meta.tx.outputs
      // Check each output to see if it is a coin for this address
      for (let i = 0; i < outputs.length; i++) {
        const address = outputs[i].getAddress();

        if (address && addr.toString() == address.toString()) {
          const coin = this.getCoin(meta.tx.hash(), i);
          if (coin)
            coins.push(coin);
        }
      }
  ...
  getTXByAddress(addr, options) {
    return this.getMetaByAddress(addr, options).map(m => m.tx);
  }
  ...
  async insertTX(tx, id) {
    ...
    if (this.chain.state.hasWellington()) {
  ...
  async verify(entry, view) {
    ...
    // After wellington is activated, remove mempool chained transaction limit
    if (!this.chain.state.hasWellington()) {
  ...
  async verifyInputs(tx, view, flags) {
    ...
    const mandatoryFlags = (flags & ~Script.flags.ONLY_STANDARD_VERIFY_FLAGS);
    ...
    if (flags & Script.flags.ONLY_STANDARD_VERIFY_FLAGS) {
      ...
      if (await tx.verifyAsync(view, mandatoryFlags ^ Script.flags.VERIFY_REVERSEBYTES, this.workers)) {
	throw new VerifyError(tx,
          'invalid',
  ...
  /**
   * Index SLP records for tx.
   * @private
   * @param {MempoolEntry} entry
   */

  async indexSlp(entry) {
    const tx = entry.tx;
    const prefix = Buffer.from('slp', 'utf8');
    const hash = tx.hash();
    const key = Buffer.concat([prefix, hash]);
    // Validate SLP
    const { type, records } = this.slpindex.buildSlpRecords(tx);
    if (records.length == 0)
      return;
    
    // Add GENESIS straight away, no validation needed
    if (type == 'GENESIS') {
      this.map.set(key, records);
      this.logger.debug(
        'Indexed SLP Records for %h in mempool.',
        hash);
    } else {
      // Sum up output totals
      // console.log('Processing SLP token...')
      const { tokenId, version } = records[0];
      // console.log('records', records);
      // console.log('tokenId', tokenId);
      const outputTotal = U64.fromBE(Buffer.alloc(8));
      for (const {value} of records ) {
        if (value)
          outputTotal.iadd(U64.fromBE(value));
      }
      // Get input totals for this token
      const inputTotal = U64.fromBE(Buffer.alloc(8));
      let hasBaton = false
      for (const {prevout} of tx.inputs) {
        const { hash, index } = prevout;
        const inKey = Buffer.concat([prefix, hash]);
        const inputRecords = this.map.get(inKey) || [];
        const inputRecord = inputRecords.find(r => r.vout == index)
          || (await this.slpindex.getSlpCoinRecords(hash, index));

        // Be sure same token is being spent as is being output
        if (inputRecord) {
          // console.log('inputRecord.tokenId', inputRecord.tokenId);
          // Verify same tokenId
          if (Buffer.compare(inputRecord.tokenId, tokenId) !== 0)
              continue;

          if (inputRecord.type == 'BATON')
            hasBaton = true;
          else if (inputRecord.type) {
            // If it is one of the other types, sum up input totals
            if (inputRecord.value)
              inputTotal.iadd(U64.fromBE(inputRecord.value))
          }
        }

        // Check to see if this is a valid token type 2 mint vault baton
        if (version === 2 && type == 'MINT') {
          // Clone the tokenId buffer before reversing it in place
          const tokenIdHash = Buffer.from(tokenId.toString('hex'), 'hex').reverse();
          // Get token record
          const tokenRecord = await this.slpindex.getTokenRecord(tokenIdHash);
          // Get the address hash for this input
          let tx = this.getTX(hash);
          if (!tx)
            tx = await this.slpindex.getTX(hash);
      ...
      // console.log('inputTotal', inputTotal.toString());
      // console.log('outputTotal',outputTotal.toString());
      const isValidMint = hasBaton && records.find(r => r.type == 'MINT');
      if (inputTotal.gte(outputTotal) || isValidMint) {
        this.map.set(key, records);
        this.logger.debug(
          'Indexed SLP Records for %h in mempool.',
          hash);
